...
mapdat <- read.csv("index/www/data/site_locations.csv") %>%
  filter(site.code != "otpo" & site.code != "scmo") %>% 
...
species_leaflet <- function (x) {
  ...
  map <- leaflet(options = leafletOptions(zoomControl = FALSE)) %>% 
    ...
    addMarkers(formap$longitude, formap$latitude, label = formap$site) #%>%
...
bardat <- read.csv("index/www/data/site_locations.csv") %>%
  filter(site != "" & site != "otter_point" & site != "schoodic_mountain") %>% 
...
<div class = "bodybody">
  ...
  <div class = "highlight-text"> `r round((length(sampsites$site.code)/10)*100, digits = 0)`% </div>
